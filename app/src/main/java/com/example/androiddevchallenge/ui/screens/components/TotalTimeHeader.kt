...
@Composable
fun TotalTimeHeader(totalTime: Long) {
    val formattedTime = getFormattedTime(millis = totalTime)

    Column(
        modifier = Modifier.fillMaxWidth().padding(top = 32.dp, bottom = 16.dp),
        horizontalAlignment = Alignment.CenterHorizontally
    ) {
        Text(text = "T I M E", style = MaterialTheme.typography.subtitle2, color = green)
